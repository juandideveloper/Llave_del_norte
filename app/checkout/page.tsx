...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    fetch("/api/colombia")
      ...
      .then((data) =>
        setDepartamentos(
          data.sort((a: Departamento, b: Departamento) =>
  ...
  const cargarCiudades = useCallback(async (id: number) => {
    ...
    try {
      ...
      setCiudades(
        data.sort((a: Ciudad, b: Ciudad) => a.name.localeCompare(b.name)),
      );
    } catch {
      setCiudades([]);
    } finally {
  ...
  useEffect(() => {
    if (departamentoId && departamento !== "Selecciona tu departamento") {
      cargarCiudades(departamentoId);
      const esBogota = departamento.toLowerCase().includes("bogot");
      setCostoEnvio(esBogota ? 0 : 15000);
    }
  ...
  async function handleContraentrega() {
    ...
    try {
      ...
      setProcesandoContraentrega(false);
    }
  }
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-8">
          ...
          {pasos.map((paso, i) => (
            <div key={i} className="flex items-center gap-2 md:gap-4">
              <div className="flex flex-col items-center">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all ${i <= pasoActual ? "bg-verde border-verde" : "bg-white border-amarillo"}`}
                >
                  {i < pasoActual ? (
                    <svg
                      width="16"
                    ...
                    <span className="text-sm font-medium text-amarillo">
                      {i + 1}
                    </span>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 text-sm font-medium rounded-lg transition-opacity flex items-center justify-center gap-2 ${aceptaTerminos ? "bg-verde text-amarillo hover:opacity-90 cursor-pointer" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
                    >
                      Continuar
                      <svg
                        width="16"
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="border border-amarillo bg-verde-medio rounded-xl p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <p className="text-md font-semibold text-amarillo">
                            Bold - Pasarela de pagos
                          </p>
                          <p className="text-sm text-gray-300">
                            Tarjeta débito / crédito, PSE y más
                          </p>
                        </div>
                        <span className="text-xs bg-amarillo text-hueso px-2 py-1 rounded-full font-medium">
                          Seguro
                        </span>
                      </div>
                      {boldIntegrity ? (
                    ...
                    <div className="border border-gray-200 bg-white rounded-xl p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <p className="text-md font-semibold text-verde">
                            Pago contraentrega
                          </p>
                      ...
                      <button
                        ...
                        disabled={procesandoContraentrega}
                        className="w-full py-3 bg-verde text-amarillo text-sm font-bold rounded-lg hover:opacity-90 transition-opacity cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                        {procesandoContraentrega ? (
                          <>
          ...
          <div>
            <ResumenPedido
              items={items}
              totalPrecio={totalPrecio as number}
              esEspecial={esEspecial}
              costoEnvio={metodoEnvio === "tienda" ? 0 : costoEnvio}
            />
          </div>
        </div>
      </div>
