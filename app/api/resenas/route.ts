...
export async function POST(req: Request) {
  ...
  if (!alegraProductoId || !calificacion)
    return NextResponse.json({ error: "Faltan datos" }, { status: 400 });

  const clienteId = Number(session.user.id);

  // Buscar pedidos pagados del cliente y revisar si el producto está en productosJson
  const pedidosPagados = await prisma.pedido.findMany({
    where: { clienteId, estadoPago: "PAGADO" },
    select: { productosJson: true },
  });

  const haComprado = pedidosPagados.some((pedido) => {
    if (!pedido.productosJson) return false;
    try {
      const items: { id: number }[] = JSON.parse(pedido.productosJson);
      return items.some((item) => String(item.id) === String(alegraProductoId));
    } catch {
      return false;
    }
  });
