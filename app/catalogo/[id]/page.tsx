...
function SkeletonDetalle() {
  return (
    <div className="max-w-8xl mx-auto px-4 md:px-6 py-6 animate-pulse">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
        <div className="rounded-t-lg bg-gray-200 h-96"/>
        <div className="bg-gray-100 border border-gray-200 rounded-t-lg p-6 space-y-4">
...
export default function DetalleProductoPage() {
  ...
  const [relacionados, setRelacionados] = useState<ProductoAlegra[]>([]);
  const [mostrarModal, setMostrarModal] = useState(false);
  const especificacionesRef = useRef<HTMLDivElement>(null);
  const sliderRelacionadosRef = useRef<HTMLDivElement>(null);
  ...
  if (cargando) {
    return (
      <div className="min-h-screen bg-white">
        <Navbar breadcrumb={[{ label: "Inicio", href: "/" }, { label: "Catálogo", href: "/catalogo" }]}/>
        <SkeletonDetalle/>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <Link href="/catalogo" className="px-4 py-2 bg-verde text-amarillo rounded-lg text-sm">Volver al catálogo</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
          ...
          <div className="rounded-t-lg bg-gray-100 border-gray-400 border-2">
            <div className="rounded-t-lg overflow-hidden h-72 md:h-96 flex items-center justify-center mb-3 bg-white">
              {imagenPrincipal ? (
                <img src={imagenPrincipal} alt={producto.name} className="object-contain h-full w-full"/>
              ) : (
                <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.8" className="text-verde/20">
            ...
            {imagenes.length > 0 && (
              <div className="flex gap-2 items-center justify-center mb-6 flex-wrap px-2">
                {imagenes.slice(0, 5).map((img, i) => (
                  <button key={img.id} onClick={() => setImagenActual(i)}
                    ...
                    <img src={img.url} alt={`${producto.name} ${i + 1}`} className="object-contain w-full h-full"/>
                  </button>
                ))}
              </div>
            )}
          ...
          {/* Info */}
          <div className="bg-gray-100 border-gray-400 border-2 rounded-t-lg pt-4 px-4 md:pl-7 md:pr-7">
            <div className="flex justify-between items-start mb-1">
              ...
              <button onClick={() => setGuardado(!guardado)}
                ...
                <svg width="25" height="25" viewBox="0 0 24 24" fill={guardado ? "#e1b86b" : "none"} stroke={guardado ? "#e1b86b" : "currentColor"} strokeWidth="1.5">
            ...
            {esEspecial ? (
              <div className="mb-4">
                <p className="text-sm text-gray-400 line-through">$ {Math.round(precio).toLocaleString("es-CO")} und</p>
                <p className="text-2xl font-medium text-amarillo">$ {precioEspecial.toLocaleString("es-CO")} und</p>
              </div>
            ) : (
              <p className="text-xl md:text-2xl font-semibold text-verde/88 mb-4">$ {Math.round(precio).toLocaleString("es-CO")} und</p>
            )}
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                ...
                <button onClick={() => setCantidad(Math.min(stock, cantidad + 1))}
                  className="w-9 h-9 flex items-center justify-center text-verde hover:bg-gray-50 transition-colors cursor-pointer">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button disabled={!botonDesbloqueado}
                  className={`px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${botonDesbloqueado ? "bg-verde text-amarillo hover:opacity-90 cursor-pointer" : "bg-red-800 text-gray-400 cursor-not-allowed"}`}>
                  Comprar ahora
                </button>
              </div>
            </div>
                ...
                <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                  <div className="px-4 pb-4 space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">{producto.description || "Sin descripción disponible"}</p>
                    </div>
                      ...
                      <tbody>
                        {[
                          ...
                          <tr key={label} className={bg}>
                            ...
                            <td className="py-2 px-4 text-gray-400">{valor}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        {relacionados.length > 0 && (
          <div className="mb-10">
            ...
            <div className="relative px-8">
              ...
              <button onClick={() => setSliderRelacionados(prev => Math.max(0, prev - 1))}
              ...
              <button onClick={() => setSliderRelacionados(prev => Math.min(Math.max(relacionados.length - 4, 0), prev + 1))}
      ...
      <AnimatePresence>
        {mostrarFlotante && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
            ...
            <button disabled={!botonDesbloqueado}
              className={`px-4 py-2 text-sm font-medium rounded-xl transition-opacity ${botonDesbloqueado ? "bg-amarillo text-verde hover:opacity-90 cursor-pointer" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
              Comprar ahora
            </button>
          </motion.div>
        )}
