        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1">
            ...
            <input
              ...
              onChange={(e) => {
                const valor = e.target.value.replace(/[^0-9]/g, "");
                setPrecioMax(Number(valor) || 50000000);
              }}
              className="w-full px-2 py-1.5 border border-gray-400 rounded-md text-xs text-gray-600 outline-none focus:border-verde bg-white"
            />
          </div>
...
function CatalogoContenido() {
  ...
  useEffect(() => {
    fetch("/api/productos")
      ...
      .then((data) => {
        if (data.productos) {
          const activos = data.productos.filter(
            (p: ProductoAlegra) => p.status === "active",
          );
            ...
            (p: ProductoAlegra) =>
              p.customFields?.some(
                (f: { name: string; value: string }) =>
                  f.name === "Recomendados" && f.value === "Si",
              ) && p.status === "active",
          );
          ...
          <div className="bg-[linear-gradient(to_bottom,#112221,#005752,#80A8A6,#E4E4E4)] rounded-xl p-4 md:p-6">
            ...
            <div className="relative px-8">
                ...
                >
                  {cargando
                    ? [1, 2, 3, 4].map((i) => (
                        <div
                          key={i}
                          className="min-w-[calc(50%-6px)] md:min-w-[calc(25%-9px)] bg-white rounded-lg overflow-hidden flex-shrink-0 animate-pulse"
                        >
                    ...
                    : recomendados.length > 0
                      ...
                      : placeholders.map((i) => (
                          <div
                            key={i}
                            className="min-w-[calc(50%-6px)] md:min-w-[calc(25%-9px)] bg-white rounded-lg overflow-hidden flex-shrink-0"
                          >
              ...
              <button
                onClick={() =>
                  setSliderActual((prev) =>
                    prev === 0
                      ? Math.max(recomendados.length - 1, 0)
                      : prev - 1,
                  )
                }
              ...
              <button
                onClick={() =>
                  setSliderActual((prev) =>
                    prev === Math.max(recomendados.length - 1, 0)
                      ? 0
                      : prev + 1,
                  )
                }
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                  {productosPaginados.map((producto) => {
                    const precio =
                      producto.price[0]?.precioConIva ??
                      producto.price[0]?.price ??
                      0; // <-- con IVA
                    ...
                    return (
                      ...
                      >
                        <Link href={`/catalogo/${producto.id}`} className="relative h-36 md:h-44 bg-white block">
