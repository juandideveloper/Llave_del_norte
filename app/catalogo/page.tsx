    ...
    <div className="space-y-4">
      <div className="bg-gray-100 rounded-xl p-4 border border-gray-400">
        ...
        <div className="space-y-2">
          {categorias.map((cat) => (
            <label key={cat.nombre} className="flex items-center justify-between gap-2 cursor-pointer group">
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={categoriaActiva === cat.nombre}
                  onChange={() => setCategoriaActiva(cat.nombre)}
                  className="accent-verde w-3.5 h-3.5 cursor-pointer"/>
                <span className={`text-xs transition-colors ${categoriaActiva === cat.nombre ? "text-verde font-medium" : "text-gray-500 group-hover:text-verde"}`}>
                  {cat.nombre}
                </span>
              </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1">
            ...
            <input type="text" placeholder="$ 0"
              value={precioMin === 0 ? "" : `$ ${precioMin.toLocaleString("es-CO")}`}
              onChange={(e) => { const valor = e.target.value.replace(/[^0-9]/g, ""); setPrecioMin(Number(valor)); }}
      ...
      <div className="bg-gray-100 rounded-xl p-4 border border-gray-400">
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={soloEnStock} onChange={(e) => setSoloEnStock(e.target.checked)}
              className="accent-verde w-3.5 h-3.5 cursor-pointer"/>
        ...
        <div className="space-y-2">
          {materiales.map((mat) => (
            <label key={mat} className="flex items-center gap-2 cursor-pointer group">
              <input type="checkbox" checked={materialesActivos.includes(mat)}
                onChange={() => toggleMaterial(mat)}
                className="accent-verde w-3.5 h-3.5 cursor-pointer"/>
              <span className={`text-xs transition-colors ${materialesActivos.includes(mat) ? "text-verde font-medium" : "text-gray-500 group-hover:text-verde"}`}>
                {mat}
              </span>
            </label>
          ))}
...
export default function CatalogoPage() {
  ...
  const [recomendados, setRecomendados] = useState<ProductoAlegra[]>([]);
  ...
  const [cardWidth, setCardWidth] = useState(200);
   // Calcular el ancho de cada card para el slider


useEffect(() => {
  if (sliderRef.current) {
    setCardWidth((sliderRef.current.offsetWidth / 4) - 9)
  }
}, [recomendados])
      ...
      .then((data) => {
        if (data.productos) {
          ...
          const recomendadosFiltrados = data.productos.filter(
            (p: ProductoAlegra) => p.category?.name === "Ventas" && p.status === "active"
          )
  ...
  const categorias = categoriasUnicas.map((nombre) => ({
    nombre,
    cantidad: nombre === "Todos los productos"
      ? productosAlegra.length
      : productosAlegra.filter((p) => (p.category?.name || "General") === nombre).length,
  }));
  ...
  function toggleMaterial(material: string) {
    setMaterialesActivos((prev) => prev.includes(material) ? prev.filter((m) => m !== material) : [...prev, material]);
    setPaginaActual(1);
  }
...
const offsetSlider = sliderActual * (cardWidth + 12);
  return (
    <div className="min-h-screen bg-white">
      <Navbar breadcrumb={[{ label: "Inicio", href: "/" }, { label: "Catalogo" }]}/>
      ...
      <div className="md:hidden px-4 py-3 bg-white border-b border-gray-100">
        <button onClick={() => setFiltrosAbiertos(!filtrosAbiertos)}
          ...
          Filtros
          {(materialesActivos.length > 0 || soloEnStock || categoriaActiva !== "Todos los productos") && (
            <span className="bg-verde text-amarillo text-xs w-5 h-5 rounded-full flex items-center justify-center">
              {materialesActivos.length + (soloEnStock ? 1 : 0) + (categoriaActiva !== "Todos los productos" ? 1 : 0)}
            </span>
          )}
        </button>
      </div>
...
<div className="bg-[linear-gradient(to_bottom,#112221,#005752,#80A8A6,#E4E4E4)] rounded-xl p-4 md:p-6">
  <h2 className="text-center text-xs font-medium text-hueso/60 uppercase tracking-widest mb-4">Recomendados</h2>
  <div className="relative px-8">
    <div className="overflow-hidden" ref={sliderRef}>
      <div
        ...
        {cargando ? [1,2,3,4].map((i) => (
          <div key={i} className="min-w-[calc(50%-6px)] md:min-w-[calc(25%-9px)] bg-white rounded-lg overflow-hidden flex-shrink-0 animate-pulse">
          ...
          return (
            <Link key={prod.id} href={`/catalogo/${prod.id}`}
              className="min-w-[calc(50%-6px)] md:min-w-[calc(25%-9px)] bg-white rounded-lg overflow-hidden flex-shrink-0 cursor-pointer hover:border-amarillo border border-transparent transition-colors">
              <div className="relative h-32 md:h-36 bg-gray-50">
                {imagenUrl ? (
              ...
              <div className="p-2 md:p-3">
                <p className="text-xs font-medium text-verde mt-1 leading-tight line-clamp-2">{prod.name}</p>
                <p className="text-xs font-medium text-verde mt-1">$ {Math.round(precio).toLocaleString("es-CO")} und</p>
              </div>
            </Link>
          )
        }) : placeholders.map((i) => (
          <div key={i} className="min-w-[calc(50%-6px)] md:min-w-[calc(25%-9px)] bg-white rounded-lg overflow-hidden flex-shrink-0">
    ...
    <button onClick={() => setSliderActual(prev => prev === 0 ? Math.max(recomendados.length - 1, 0) : prev - 1)}
    ...
    <button onClick={() => setSliderActual(prev => prev === Math.max(recomendados.length - 1, 0) ? 0 : prev + 1)}
    ...
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
        {productosPaginados.map((producto) => {
          ...
          const enStock = (producto.inventory?.availableQuantity || 0) > 0;
          const imagenUrl = producto.images?.find(img => img.favorite)?.url || producto.images?.[0]?.url || null;

          return (
            <motion.div key={producto.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="relative h-36 md:h-44 bg-gray-50">
                ...
                {!enStock && (
                  <div className="absolute top-2 right-2 z-10 bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">
                    Agotado
                  </div>
                )}
              </div>
              <div className="p-3">
                ...
                {esEspecial ? (
                  <div className="mt-2">
                    ...
                    <p className="text-sm font-semibold text-verde">$ {precioEspecial.toLocaleString("es-CO")} und</p>
                    <p className="text-xs text-gray-400 mt-1">Este precio se mantiene ya que eres mayorista</p>
                  </div>
                ) : (
                  <p className="text-sm font-medium text-verde mt-2">$ {Math.round(precio).toLocaleString("es-CO")} und</p>
                )}
      ...
      {totalPaginas > 1 && (
        <div className="flex items-center justify-center gap-2 mt-8">
          ...
          {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((pagina) => {
            const mostrar = pagina === 1 || pagina === totalPaginas || Math.abs(pagina - paginaActual) <= 1;
            if (!mostrar) {
              const anteriorMostrada = pagina - 1 === 1 || pagina - 1 === totalPaginas || Math.abs(pagina - 1 - paginaActual) <= 1;
              if (!anteriorMostrada) return null;
              return <span key={pagina} className="w-9 h-9 flex items-center justify-center text-sm text-gray-400">...</span>;
            }
            return (
              <button key={pagina} onClick={() => setPaginaActual(pagina)}
                className={`w-9 h-9 rounded-xl border text-sm font-medium transition-colors cursor-pointer ${
                  paginaActual === pagina ? "bg-verde text-amarillo border-verde" : "border-gray-200 text-gray-600 hover:border-verde hover:text-verde"
                }`}>
                {pagina}
              </button>
            );
